...
export default function ModuloForm() {
    ...
    const setArrayTextField = useCallback(
        (key: 'hora_1' | 'hora_2' | 'hora_3', index: number, value: string) => {
            setForm((prev) => {
                const arr = [...prev[key]]
                arr[index] = value
                return { ...prev, [key]: arr }
        ...
        async (download: boolean) => {
            ...
            try {
                ...
                const msg = axios.isAxiosError(err)
                    ? err.response?.data?.detail || 'No se pudo generar CD.'
                    : 'No se pudo generar CD.'
                toast.error(msg)
            } finally {
                setLoading(false)
            }
    ...
    return (
        <div className="min-h-screen bg-slate-100 p-4 md:p-6">
            <div className="mx-auto max-w-[1200px] space-y-4">
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-300 bg-white shadow-sm">
                        ...
                        <table className="w-full table-fixed border border-slate-300 text-sm">
                            <thead className="bg-slate-100 text-xs font-semibold text-slate-800">
                                <tr>
                                    ...
                                    <th className="border-r border-slate-300 py-1" colSpan={2}>N° OT</th>
                                    <th className="py-1" colSpan={2}>FECHA</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                    ...
                    <div className="border-b border-slate-300 bg-slate-100 px-4 py-3 text-center">
                        ...
                        <p className="text-[13px] font-semibold text-slate-900">NORMA NTP 339.171</p>
                    </div>
                    ...
                    <div className="p-3">
                        <table className="w-full table-fixed border border-slate-300 text-sm">
                            <thead className="bg-slate-50 text-xs font-semibold text-slate-800">
                                ...
                                <tr>
                                    <th className="border-r border-slate-300 py-1">Def. Horizonta</th>
            ...
            <FormatConfirmModal
                ...
                formatLabel={`Formato N-xxxx-SU-${new Date().getFullYear().toString().slice(-2)} CD`}
